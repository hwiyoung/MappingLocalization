...
for region in regions:
    ...
    for idx_q in range(len(query_imgs)):
        ...
        for idx_d in range(db_imgs[idx_q]-seq, db_imgs[idx_q]+seq):
            ...
            images_to_process[2*i] = os.path.join(db_dir, db_L_idx)
            images_to_process[2*i+1] = os.path.join(db_dir, db_R_idx)
            i += 1
        ...
        print("Align photos")
        ba_api = BA_api()
        cam_L, eo_L, cam_R, eo_R = ba_api.alignphotos_2(images_to_process, region)
