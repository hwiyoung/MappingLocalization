...
for region in regions:
    ...
    for idx_q in range(len(query_imgs)):
        f = open(region + str(idx_q).zfill(2) + ".txt", "w")

        query_path = glob.glob(os.path.join(query_dir, region + str(idx_q).zfill(2)) + "/*.png")

        interval = 10
        test_img = query_path[-2].split("\\")[1] + query_path[-2].split("\\")[2].zfill(9)
        # test|train|train-20|train-10|train+10|train+20
        images_to_process = [test_img, str(db_imgs[idx_q]).zfill(6) + ".png",
