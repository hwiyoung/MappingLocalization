...
# 1. Load a csv file
reader = np.loadtxt("retrieved naverlabs pangyo image indices.csv", dtype=np.int, delimiter=",")

# 2. Match the closest db images corresponding to query images
query_imgs = reader[:, 0]
db_imgs = reader[:, 1]

query_dir = "Z:/Mapping_Localization/outdoor/outdoor_dataset/outdoor_main/pangyo/test"
query_L_path = ["" for i in range(len(query_imgs))]
query_R_path = ["" for i in range(len(query_imgs))]
for idx_q in range(len(query_imgs)):
    query_L_idx = "pangyo" + str(idx_q).zfill(2) + "/049_L.png"
    query_R_idx = "pangyo" + str(idx_q).zfill(2) + "/049_R.png"
    ...
    db_L_dir = "Z:/Mapping_Localization/outdoor/outdoor_dataset/outdoor_main/pangyo/train/images/left"
    db_R_dir = "Z:/Mapping_Localization/outdoor/outdoor_dataset/outdoor_main/pangyo/train/images/right"
    seq = 25
    i = 0
    ...
    print("Align photos")
    ba_api = BA_api()
    cam_L, eo_L, cam_R, eo_R = ba_api.alignphotos(images_to_process)
